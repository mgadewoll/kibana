...
// Escludes addError on purpose as it has a different API
export const TOAST_METHODS = new Set(['addSuccess', 'addWarning', 'addInfo', 'addDanger']);

// Supported utils used in Toast API `text` to render JSX
export const MOUNT_FUNCTIONS = new Set(['mountReactNode', 'toMountPoint']);

// Interactive elements that should be passed via `actionProps` and not as text content
export const ACTION_ELEMENTS = new Set([
  'EuiButton',
  'EuiButtonEmpty',
  'EuiButtonIcon',
  'EuiButtonGroup',
...
export const getJSXElementName = (name: TSESTree.JSXTagNameExpression): string | null => {
 ...
 */
export const collectMountContent = (rootNode: TSESTree.Node): ContentFindings => {
...
export interface ToastMountArg {
  methodName: string;
  mountFn: string;
 ...
 */
export const forEachToastMountArg = (
  node: Rule.Node,
